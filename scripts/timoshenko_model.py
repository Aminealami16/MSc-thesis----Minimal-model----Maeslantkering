...
## Output: matrix of 12x12 containing the mass matrix of a three-dimensional 2-node Timoshenko beam element
      ...
      # added mass due to oscillation of the water around the retaining wall
       ...
       me_oscillation = Ca * rhow * b * h
       Me_oscillation = np.zeros((12,12))
       Me_oscillation[0:6,0:6] = me_oscillation * Me11
       Me_oscillation[6:,0:6]  = me_oscillation * Me21
       Me_oscillation[0:6,6:]  = me_oscillation * Me21.T
       Me_oscillation[6:,6:]   = -me_oscillation * Me11 + 2 * me_oscillation * np.diag(np.diagonal(Me11))

       L_truss = 237.5
       A_truss = np.pi * (0.5 * 1.75)**2
       me_truss = rho * A_truss * 0.5 * L_truss /  (16) # Number of elements
       Me_truss = np.zeros((12,12))
       Me_truss[0:6,0:6] = me_truss * Me11
       Me_truss[6:,0:6]  = me_truss * Me21
       Me_truss[0:6,6:]  = me_truss * Me21.T
       Me_truss[6:,6:]   = -me_truss * Me11 + 2 * me_truss * np.diag(np.diagonal(Me11))
       ...
       if type == 'normal':
              ...
              return Me + Me_added
       elif type == 'added mass oscillation':
              return Me + Me_added + Me_oscillation
       elif type == 'added mass truss':
              return Me + Me_truss
...
def T_element_added_mass_oscillation_retaining_wall(ex,ey,ez,eo,ep_K,ep_M):
...
## Output: matrices of 12x12 containing the mass and stiffness matrices of a three-dimensional 2-node Timoshenko beam element
       ...
       K_l = K_local(ex, ey, ez, ep_K)
       M_l = M_local(ex, ey, ez, ep_M, type='added mass oscillation')
       G   = rotation(ex,ey,ez,eo)
...
def T_element_added_mass_truss(ex,ey,ez,eo,ep_K,ep_M):
## Function that contains the mass and stiffness matrices for a Timoshenko beam element in global coordinate system
## Input:
              ## ex = [x1, x2] [m]
              ## ey = [y1, y2] [m]
...
## Output: matrices of 12x12 containing the mass and stiffness matrices of a three-dimensional 2-node Timoshenko beam element
       K_l = K_local(ex, ey, ez, ep_K)
       M_l = M_local(ex, ey, ez, ep_M, type='added mass truss')
       G   = rotation(ex,ey,ez,eo)
